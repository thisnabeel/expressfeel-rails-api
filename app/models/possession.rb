class Possession < ActiveRecord::Base
    ...
    def self.build(size)
        output = []
        Possession.all.sample(size).each do |c|
            lang = c.language
            noun = lang.language_nouns.sample.id
            output.push(c.build(noun))
        end
        return output
    end

    def build(language_noun_id)
        language_noun = LanguageNoun.find(language_noun_id)
        @@language_noun = language_noun
        english_noun = language_noun.noun

        puts "++++++++++++++"
        puts language_noun.attributes
        puts "++++++++++++++"
        ...
        self.english.match(/\(([^\)]+)\)/).try(:captures).try(:each) do |match|
            english = self.english.gsub("(#{match})", english_noun[match])
        end

        self.roman.match(/\(([^\)]+)\)/).try(:captures).try(:each) do |match|
            puts match
            puts language_noun.folder
            puts language_noun.folder[match]
            roman = self.roman.gsub("(#{match})", polished(language_noun.folder[match], roman_instructions))
        end

        self.original.match(/\(([^\)]+)\)/).try(:captures).try(:each) do |match|
            original = self.original.gsub("(#{match})", polished(language_noun.folder[match], original_instructions))
        end
        ...
        hash = {
            ...
            "meta" => {
                ...
                "language_noun_id" => language_noun.id,
            }
        }
    end
    ...
    def polished(word, instructions)
        ...
        instructions.each do |instruction|
            ...
            instruction["conditions"].each do |condition|
                ...
                passing = true
                passing = passing? @@language_noun.folder, condition["ifs"] if condition["ifs"].present?
